...
function like_product(p_user_id user.user_id%type, p_product_name product.product_name%type);

function like_brand(p_user_id user.user_id%type, p_product_brand product.p_product_brand%type);

function like_ingredient(p_user_id user.user_id%type, p_ingredient_name ingredients.ingredient_name%type);


function delete_like_product(p_user_id user.user_id%type, p_product_name product.product_name%type);

function delete_like_brand(p_user_id user.user_id%type, p_product_brand product.p_product_brand%type);

function delete_like_ingredient(p_user_id user.user_id%type, p_ingredient_name ingredients.ingredient_name%type);
...
function delete_like_product(p_user_id user.user_id%type, p_product_name product.product_name%type) IS
return varchar2
...
function delete_like_brand(p_user_id user.user_id%type, p_product_brand product.p_product_brand%type) IS
return varchar2
...
function delete_like_ingredient(p_user_id user.user_id%type, p_ingredient_name ingredients.ingredient_name%type) IS
return varchar2
...
if(v_validate==1) then
	delete from user_likes where upper(product_ingredient_name)=upper(p_product_name) and user_id=p_user_id;
	v_message:='succefully deleted';
else
	v_message:='not such a product';
END if;
